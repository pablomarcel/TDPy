...
Run without hardware (four UI profiles):
    # Classic ttk
    ...
    # LabVIEW-style (industrial 'front panel' look, silver/graphite + amber)
    PYCONTROL_LABUI=1 PYCSB_DEMO=1 python pycontrol.py
    # or: PYCONTROL_UI=lab
"""
...
USE_LABUI = os.getenv("PYCONTROL_LABUI", "0") == "1" or ENV_UI in ("lab", "labview", "lv", "ni", "lab-view")
USE_MACUI = (os.getenv("PYCONTROL_MACUI", "0") == "1" or ENV_UI in ("mac", "macos")) and not USE_LABUI

# Optional ttkbootstrap only when explicitly enabled (ignored if Mac/Lab UI selected)
USE_TTKB = (os.getenv("PYCONTROL_TTKB", "0") == "1") and not USE_MACUI and not USE_LABUI
if USE_TTKB:
    try:
...
DEFAULT_PREFS = {
    "theme": "clam",     # classic ttk look (ignored for bootstrap/mac/lab)
    "baud": 9600,
    "palette": "Matlab",
    "ui_profile": "lab" if USE_LABUI else ("mac" if USE_MACUI else ("bootstrap" if USE_TTKB else "classic")),
}
...
PALETTES = {
    ...
    # LabVIEW-like: dark front-panel, soft grid, amber trace, light text
    "LabVIEW":   ("#1f2125", "#3a3f44", "#f2c200", "#f2f2f2", "#7a7f85"),
}
...
class PyControlApp(tk.Tk):
    def __init__(self) -> None:
        ...
        # palette
        default_palette = "LabVIEW" if self.ui_profile == "lab" else ("MacOS" if self.ui_profile == "mac" else self.prefs.get("palette", "Matlab"))
    ...
    def _init_style(self, theme_name: str) -> Tuple[ttk.Style, str]:
        """
        Returns (style, ui_profile) where ui_profile ∈ {'classic','bootstrap','mac','lab'}.
        Precedence: Lab > Mac > Bootstrap > Classic.
        """
        # --- LabVIEW-style profile ------------------------------------------------
        if USE_LABUI:
            ui_profile = "lab"
            style = ttk.Style()
            try:
            ...
            # Industrial front-panel neutrals
            window_bg = "#dfe1e5"   # light silver
            panel_bg  = "#d0d2d6"   # slightly darker for cards
            control_bg = "#f2f3f5"  # control face
            graphite   = "#1e1f22"  # text
            border     = "#8b8f94"
            accent     = "#f2c200"  # NI-ish amber/yellow

            # fonts (Lab-like blocky look; use Verdana fallback)
            def _tweak_fonts():
                try:
                    base = tkfont.nametofont("TkDefaultFont")
                    base.configure(family="Verdana", size=11)
                except Exception:
                    pass
            ...
            style.configure(".", background=window_bg, foreground=graphite)
            style.configure("TFrame", background=window_bg)
            style.configure("TLabel", background=window_bg, foreground=graphite)
            # Card frames look like LabVIEW group boxes
            style.configure("Card.TLabelframe",
                            background=panel_bg, bordercolor=border,
                            relief="groove", borderwidth=1, padding=6)
            style.configure("Card.TLabelframe.Label",
                            background=panel_bg, foreground=graphite,
                            font=("Verdana", 11, "bold"))

            # Entries and combos: recessed
            style.configure("TEntry", fieldbackground=control_bg, background=control_bg,
                            foreground=graphite, bordercolor=border,
                            lightcolor=border, darkcolor=border, borderwidth=1, padding=3)
            style.configure("TCombobox", fieldbackground=control_bg, background=control_bg,
                            bordercolor=border, lightcolor=border, darkcolor=border,
                            arrowcolor=graphite)

            # Buttons: rectangular, amber accent variant
            style.configure("TButton", background=control_bg, foreground=graphite,
                            relief="raised", borderwidth=1, padding=(10, 6))
            style.map("TButton",
                      relief=[("pressed", "sunken")],
                      background=[("active", "#e9ebee")])

            style.configure("Accent.TButton", background=accent, foreground="#2a2a2a",
                            padding=(10, 6))
            style.map("Accent.TButton",
                      background=[("active", "#ffd54d"), ("pressed", "#f5c400")])

            # Toolbar style
            style.configure("LabToolbar.TFrame", background=panel_bg, relief="ridge", borderwidth=1)
            style.configure("LabTool.TButton", background=control_bg, padding=(8, 4))
            style.map("LabTool.TButton",
                      background=[("active", "#eceff1"), ("pressed", "#e0e0e0")])

            # Radio / check
            style.configure("TRadiobutton", background=panel_bg, foreground=graphite)
            style.configure("TCheckbutton", background=panel_bg, foreground=graphite)
    ...
    def _build_menu(self) -> None:
        ...
        uim.add_command(label="LabVIEW-style (restart; PYCONTROL_LABUI=1)",
                        command=lambda: self._explain_switch("lab"))
    ...
    def _explain_switch(self, target: str):
        msg = (
            ...
            "  • LabVIEW-style: PYCONTROL_LABUI=1 python pycontrol.py\n"
        )
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(lf, text="PyControl",
                  style="Header.TLabel" if self.ui_profile not in ("mac", "lab") else "TLabel").pack(side=tk.LEFT, padx=8)
        ...
        tb_prefix = "Lab" if self.ui_profile == "lab" else ("Mac" if self.ui_profile == "mac" else "")
    ...
    def _make_logo_img(self):
        ...
        # frame varies slightly by profile
        frame_col = (200, 200, 205, 255) if self.ui_profile == "mac" else (150, 152, 156, 255) if self.ui_profile == "lab" else (180, 180, 180, 255)
        d.rounded_rectangle([0, 0, W - 1, H - 1], radius=6, outline=frame_col)
        # waveform color by profile
        if self.ui_profile == "mac":
            wave = (0, 122, 255, 255)
        elif self.ui_profile == "lab":
            wave = (242, 194, 0, 255)
        else:
            wave = (60, 120, 200, 255)
        x_prev, y_prev = 6, H // 2
        for x in range(6, W - 6):
            ...
            d.line([x_prev, y_prev, x, y], fill=wave, width=2)
            x_prev, y_prev = x, y
    ...
    def _about(self):
        messagebox.showinfo(
            ...
            "- Quick SysID\n\n"
            "UI profiles: classic • bootstrap • mac • labview\n"
